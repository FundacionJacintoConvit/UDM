...
function Epoch2(name, mode, targetelement, multiselect,rango) {
    ...
    function calConfig() {
        ...
        self.displayMonth = self.displayMonthInitial;
		  milisegundos = parseInt(rango * 24 * 60 * 60 * 1000);// 7 dias en milisegundos
		  fecha = new Date();
        fecha.setTime(parseInt(fecha.getTime() - milisegundos));
    ...
    function setLang() {
        ...
        self.maxrange_caption = 'Este es el rango m\xE1ximo';  //m&aacute;ximo máximo
    ...
    function createMainHeading() {
        ...
        /*var aclose = document.createElement('input');
         aclose.setAttribute('type','button');
         aclose.setAttribute('value','X');
         setClass(aclose,'aclose');
         addEventHandler(aclose,'click',function(){self.hide();});*/
    ...
    function createFooter() {
        ...
        //setClass(container,'divFooter');
        var clearSelected = document.createElement('input');
        clearSelected.style.textAlign = 'center';
    ...
    function setMode(targetelement) {
        if (self.mode == 'popup') { //set positioning to absolute for popup
            ...
            //self.calendar.style.position = 'relative';
        }
    ...
    self.setTarget = function (targetelement, focus)
    {
        ...
        if (self.mode == 'popup') {
            ...
            if (self.tgt.offsetHeight == 0) {//Porque hay veces que no toma la altura del input
                self.topOffset = 19; // the vertical distance (in pixels) to display the calendar from the Top of its input element
            } else {
    ...
    self.show = function () {
        ...
        //alert ("iframe"+self.tgt.name);
        iframe = document.getElementById("iframe" + self.tgt.name);
        //alert(iframe.name);
        iframe.style.position = "absolute";
        iframe.style.display = "block";
        iframe.style.top = self.calendar.style.top;
        iframe.style.left = self.calendar.style.left;
        iframe.style.width = "215px";
        iframe.style.height = "215px";
        self.visible = true;
    };
    //-----------------------------------------------------------------------------
    /**
    ...
    self.hide = function () {
        ...
        //alert("iframe"+self.tgt);

        //alert ("iframe"+self.tgt.name);
        document.getElementById("iframe" + self.tgt.name).style.display = 'none';
        self.visible = false;
    };
    //-----------------------------------------------------------------------------
    /**
...
/**
*	Función creada para obtener fechas futuras y no solo hasta el dia en curso
*/
function EpochFuture(name, mode, targetelement, multiselect) {
    ...
    function calConfig() {
		...
		var anios  = new_fecha.getFullYear() + 20; //usada para aumentar en años la fecha actual	
        new_fecha.setFullYear(anios);
		self.versionNumber = '2.0.2';
        self.displayYearInitial = self.curDate.getFullYear(); //the initial year to display on load
        self.displayMonthInitial = self.curDate.getMonth(); //the initial month to display on load (0-11)
        self.displayYear = self.displayYearInitial;
    ...
    function setLang() {
        ...
        self.clearbtn_caption = 'Limpiar';
        self.clearbtn_title = 'Limpiar todas las fechas seleccionadas en el calendario';
        self.maxrange_caption = 'Este es el rango m\xE1ximo';  //  máximo m&aacute;ximo;
        self.closebtn_caption = 'Cerrar';
        self.closebtn_title = 'Cierre el calendario';
    }
